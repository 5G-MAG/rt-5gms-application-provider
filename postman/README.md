...
use our pre-defined Postman collections and environment located in the `postman` folder of this repository.
...
our routes or payloads. The first variables we need to define are the `m1_url` and `m5_url` variables. For that reason,
...
Add the URL to the `M1` and the `M5` endpoint of your Application Function here. In the example above, we are running
